#!/usr/bin/env python3
"""Corrected regular Gemma 4 speed probe: disable conversation mode."""
...
N_PREDICT = os.environ.get("GEMMA4_N_PREDICT", "128")


def run(cmd: str | list[str], timeout: int = 120, cwd: Path | None = None, tail: int = 80000) -> dict[str, Any]:
    started = time.time()
    try:
        proc = subprocess.run(
            cmd,
...
def main() -> int:
    ...
    out_dir = OUT_ROOT / f"{stamp}_gemma4_nocnv_speed"
    out_dir.mkdir(parents=True, exist_ok=True)
    progress = out_dir / "progress.jsonl"
    meta: dict[str, Any] = {"prompt": PROMPT, "n_predict": N_PREDICT, "output_dir": str(out_dir)}
    ...
    try:
        ...
        append(progress, "checkout_done", fetch_returncode=meta["fetch_pr"].get("returncode"), checkout_returncode=meta["checkout"].get("returncode"))
        ...
        append(progress, "download_start")
        ...
        meta["inference"] = run(cmd, timeout=600)
        append(progress, "inference_done", returncode=meta["inference"].get("returncode"), elapsed_sec=meta["inference"].get("elapsed_sec"), error=meta["inference"].get("error"))
        (out_dir / "gemma4_nocnv.stdout.txt").write_text(meta["inference"].get("stdout", ""), encoding="utf-8")
        (out_dir / "gemma4_nocnv.stderr.txt").write_text(meta["inference"].get("stderr", ""), encoding="utf-8")
        meta["metrics"] = parse_metrics(meta["inference"].get("stdout", "") + "\n" + meta["inference"].get("stderr", ""))
    ...
    (out_dir / "gemma4_nocnv_benchmark_metadata.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (out_dir / "gemma4_nocnv_speed_report.md").write_text(
        "# Gemma 4 No-Conversation L4 Speed Probe\n\n"
        + json.dumps({"status": meta.get("status"), "gpu": meta.get("gpu"), "metrics": meta.get("metrics"), "error": meta.get("error")}, ensure_ascii=False, indent=2)
        + "\n",
        encoding="utf-8",
    )
