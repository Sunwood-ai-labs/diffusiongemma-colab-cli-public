#!/usr/bin/env python3
"""Gemma 4 GGUF completion-mode speed probe on browser-owned Colab L4."""
...
def run(cmd: str | list[str], timeout: int = 120, cwd: Path | None = None, tail: int = 120000) -> dict[str, Any]:
    started = time.time()
    try:
        proc = subprocess.run(
            cmd,
...
def main() -> int:
    ...
    out_dir = OUT_ROOT / f"{stamp}_gemma4_completion_n8_speed"
    ...
    meta["build"] = run("cmake --build build -j --config Release --target llama-completion", timeout=1800, cwd=LLAMA_DIR)
    ...
    meta["inference"] = run(cmd, timeout=900)
    append(
        progress,
        "inference_done",
        returncode=meta["inference"].get("returncode"),
        elapsed_sec=meta["inference"].get("elapsed_sec"),
        error=meta["inference"].get("error"),
    )
    (out_dir / "stdout.txt").write_text(meta["inference"].get("stdout", ""), encoding="utf-8")
    (out_dir / "stderr.txt").write_text(meta["inference"].get("stderr", ""), encoding="utf-8")
